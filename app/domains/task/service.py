...
from app.core.celery.tasks import process_workflow_event
...
from app.domains.task.repository import TaskRepository
...
class TaskService:
    ...
    async def create_task(
        db: AsyncSession, project_id, task_data: TaskCreate, current_user: User
    ):
        ...
        task = await TaskRepository.create_task(db, task)
        ...
        await WorkflowRepository.create_event(db, event)

        process_workflow_event.delay("TASK_CREATED", str(task.id))

        return task

    @staticmethod
    async def update_task_status(
        db: AsyncSession, task_id, new_status: str, current_user: User
    ):
        ...
        valid = WorkflowService.validate_transition(task.status, new_status)
        ...
        task = await TaskRepository.save(db, task)
        ...
        process_workflow_event.delay("STATUS_CHANGED", str(task.id))
